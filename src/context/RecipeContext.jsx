...
export const RecipeContextProvider = ({ children }) => {
  ...
  const fetchCuisineData = async (cuisinename) => {
    ...
    // const recipe_id=
    const data = response.hits.map((res) => ({
      ...res,
      saved: false,
      id: res.recipe.uri.slice(res.recipe.uri.lastIndexOf("_") + 1),
    }));
    setCuisines(data);
  };
  ...
  const fetchDetailedData = async (id) => {
    ...
    const data = {
      ...response,
      saved: false,
      id: response.recipe.uri.slice(response.recipe.uri.lastIndexOf("_") + 1),
    };
    setDetails(data);
  };
